...
implementation of `Brokers` and `Ref-DB Coordinator` plugins.
...
- **Ref-DB Coordinator plugin**: an implementation of the Ref-DB Coordinator interface,
...
specific choices of brokers and Ref-DB Coordinator implementations, and it does
...
verified by the Ref-DB Coordinator plugin.

* Exposing DynamicItem bindings onto which concrete _Broker_ and a _Ref-DB Coordinator_
plugins can register their specific implementations.
When no such plugins are installed, then default `Noop` bindings will be registered
instead.
...
Coordinator ensures correct behaviour of the multi-site nodes.
...
Each change attempting to mutate a ref will be checked against the Ref-DB to guarantee
that each node has an up-to-date view of the repository state.
...
See [Prevent split brain thanks to Ref-DB Coordinator](#prevent-split-brain-thanks-to-ref-db-coordinator)
For a thorough example on this.

When provided, the Ref-DB Coordinator plugin will override the dynamicItem binding
exposed by the multi-site module with a specific implementation, such as Zoekeeper,
etcd, MySQL, Mongo, etc.

#### Noop implementation
The default `Noop` implementation provided by the `Multi-site` libModule does nothing
for identifying out-of-sync refs. This is useful for setting up a test environment
and allows multi-site library to be installed independently from any additional
...
#### Prevent split brain thanks to Ref-DB Coordinator

The above scenario can be prevented by using an implementation of the Ref-DB Coordinator
