...
While this situation, at stage #7, can be experience only as a consequence of a 
failure in the RW,  it is instead the continuosly happening if moving to stage #8
...
As you can see whe have when two different clients each doing a `push` on top of 
the same branch. 
At `t0`: both clients are seeing the status of `HEAD` being `W0`. `Instance1` is the 
RW node and will receive any `push` request.

At `t1`: `Client1` push-es `W1`. The request is served by `Instance1` that acknowledges it
...
At `t4`: `Client2` push-es `W2` that is still based on `W0` (`W0 -> W2`). 
...
_Synchronous replication_ would solve the problem operating on the first of the causes, 
...
Another option is to operate on the second cause, i.e. allowing instances to realise that
_they are not in sync on a particular branch_ and refuse any operation on it. 
...
key -> value storage. Examples could be `Zookeeper`, `etcd`, `Consul` or `Hazelcast`.
...
operations on the branch can be recovered. 
...
full cluster to a Read Only state if the response from 
