...
function cleanup_environment {
  ...
  IS_DOCKER_DESKTOP=$(is_docker_desktop)
  DOCKER_HOST_ENV=$(docker_host_env $IS_DOCKER_DESKTOP)
  docker-compose -f $SCRIPT_DIR/docker-compose.yaml -f $SCRIPT_DIR/docker-compose-$DOCKER_HOST_ENV.yaml down 2> /dev/null
...
if [ $IS_KAFKA_RUNNING -lt 1 ];then
  ...
  docker-compose -f $SCRIPT_DIR/docker-compose.yaml -f $SCRIPT_DIR/docker-compose-$DOCKER_HOST_ENV.yaml up -d
