...
if [ "$BROKER_TYPE" = "kinesis" ]; then
echo "Downloading events-aws-kinesis plugin STABLE-3-3 (TODO: move to 3.4 once available)"
  wget $GERRIT_CI/plugin-events-aws-kinesis-bazel-stable-3.3/$LAST_BUILD/events-aws-kinesis/events-aws-kinesis.jar \
...
echo "Downloading events-gcloud-pubsub plugin STABLE-3-3 (TODO: move to 3.4 once available)"
  wget $GERRIT_CI/plugin-events-gcloud-pubsub-bazel-stable-3.3/$LAST_BUILD/events-gcloud-pubsub/events-gcloud-pubsub.jar \
