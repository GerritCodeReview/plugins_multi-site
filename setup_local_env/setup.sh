...
if [ $NEW_INSTALLATION = "true" ]; then
	...
	java -jar $DEPLOYMENT_LOCATION/gerrit.war init --batch --no-auto-start --install-plugin=replication --install-plugin=codemirror-editor --install-plugin=download-commands --dev -d $LOCATION_TEST_SITE_1
