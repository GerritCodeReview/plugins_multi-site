...
if [ $NEW_INSTALLATION = "true" ]; then
	...
	## Get a repo for test
	git clone --bare https://gerrit.googlesource.com/plugins/multi-site $LOCATION_TEST_SITE_1/git/multi-site.git
...
echo "Setting 'secret' as http password"
ssh -o StrictHostKeyChecking=no -p $GERRIT_1_SSHD_PORT admin@localhost gerrit set-account --http-password secret admin
ssh -o StrictHostKeyChecking=no -p $GERRIT_2_SSHD_PORT admin@localhost gerrit set-account --http-password secret admin
