...
function create_kinesis_streams {
  echo "wait for kinesis to be responsive. Sleep 5 seconds..."; sleep 5s
  ...
  do
    ...
    aws --endpoint-url=http://localhost:$BROKER_PORT kinesis create-stream --shard-count 1 --stream-name $stream
  done
}
...
function wait_until_containers_are_running {
  local log_label=$1
...
export_broker_port
wait_until_containers_are_running "core" "prometheus_test_node" "docker-compose-core.yaml"
wait_until_containers_are_running "$BROKER_TYPE" "${BROKER_TYPE}_test_node" "docker-compose-$BROKER_TYPE.yaml"
prepare_broker_data
