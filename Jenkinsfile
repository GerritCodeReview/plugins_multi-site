    ...
    def extraPlugins = [ 'pull-replication' ]
            ...
            stage('Checkout') {
                steps {
                    withCredentials([usernamePassword(usernameVariable: "GS_GIT_USER", passwordVariable: "GS_GIT_PASS", credentialsId: env.GERRIT_CREDENTIALS_ID)]) {
                        ...
                        script {
			  extraPlugins.each { plugin -> sh "[ -d gerrit/plugins/${plugin} ] || (git clone -b ${GERRIT_BRANCH} ${pluginScmBaseUrl}/plugins/${plugin} && cd gerrit/plugins && ln -sf ../../${plugin} .)" }
                        }
                    }
                }
            }
