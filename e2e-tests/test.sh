...
case "$1" in
  "--help" )
    ...
    echo "[--start-period]                Gerrit start period timeout (until it gets healthy); defaults to [${DEF_GERRIT_START_PERIOD}]"
...
GERRIT_START_PERIOD=${GERRIT_START_PERIOD:-${DEF_GERRIT_START_PERIOD}}
...
export GERRIT_IMAGE; \
  export GERRIT_START_PERIOD; \
  docker-compose ${COMPOSE_FILES} config > ${DEPLOYMENT_LOCATION}/docker-compose-setup.yaml

trap cleanup_tests_hook EXIT
docker-compose -f ${DEPLOYMENT_LOCATION}/docker-compose-setup.yaml up -d zookeeper kafka
docker-compose -f ${DEPLOYMENT_LOCATION}/docker-compose-setup.yaml ps -a
docker-compose -f ${DEPLOYMENT_LOCATION}/docker-compose-setup.yaml logs -f --no-color -t > ${DEPLOYMENT_LOCATION}/site.log &

docker-compose -f ${DEPLOYMENT_LOCATION}/docker-compose-setup.yaml up --no-start gerrit1 gerrit2
docker-compose -f ${DEPLOYMENT_LOCATION}/docker-compose-setup.yaml ps -a
GERRIT1_CONTAINER=$(docker-compose -f ${DEPLOYMENT_LOCATION}/docker-compose-setup.yaml ps -q gerrit1)
GERRIT2_CONTAINER=$(docker-compose -f ${DEPLOYMENT_LOCATION}/docker-compose-setup.yaml ps -q gerrit2)
echo "Gerrit services 1[${GERRIT1_CONTAINER}] and 2[${GERRIT1_CONTAINER}]"
...
docker-compose -f ${DEPLOYMENT_LOCATION}/docker-compose-setup.yaml up -d gerrit1 gerrit2

echo "Waiting for services to start (and being healthy) and calling e2e tests"
docker-compose -f ${DEPLOYMENT_LOCATION}/docker-compose-setup.yaml up --no-start tester
docker-compose -f ${DEPLOYMENT_LOCATION}/docker-compose-setup.yaml ps -a
TEST_CONTAINER=$(docker-compose -f ${DEPLOYMENT_LOCATION}/docker-compose-setup.yaml ps -q tester)
echo "Tester [${TEST_CONTAINER}]"
