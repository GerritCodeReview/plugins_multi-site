...
public class ProjectVersionRefUpdateTest implements RefFixture {
  ...
  @Mock RefReplicatedEvent refReplicatedEvent;
  ...
  public void producerShouldUpdateProjectVersionUponRefUpdatedEvent() throws IOException {
    ...
    when(sharedRefDb.compareAndPut(any(Project.NameKey.class), any(Ref.class), any(ObjectId.class))).thenReturn(true);
  ...
  public void consumerShouldUpdateProjectVersionUponRefReplicatedEvent() throws IOException {
    ...
    when(refReplicatedEvent.getProjectNameKey()).thenReturn(A_TEST_PROJECT_NAME_KEY);
    when(refReplicatedEvent.getRefName()).thenReturn(A_TEST_REF_NAME);
