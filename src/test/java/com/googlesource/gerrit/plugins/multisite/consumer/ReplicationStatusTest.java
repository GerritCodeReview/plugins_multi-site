...
@Mock private ProjectVersionLogger verLogger;
  ...
  public void setup() throws Exception {
    ...
    objectUnderTest =
        new ReplicationStatus(
            replicationStatusCache, Optional.of(projectVersionRefUpdate), verLogger, projectCache, Executors.newScheduledThreadPool(1),
  ...
  public void shouldUpdateReplicationLagForProject() {
	  ...
	  when(projectVersionRefUpdate.getProjectLocalVersion(eq(projectName))).thenReturn(Optional.of(projectLocalVersion));
	  when(projectVersionRefUpdate.getProjectRemoteVersion(eq(projectName))).thenReturn(Optional.of(projectRemoteVersion));
	  
	  objectUnderTest.updateReplicationLag(Project.nameKey(projectName));
	  
      assertThat(replicationStatusCache.getIfPresent(projectName)).isEqualTo(projectRemoteVersion - projectLocalVersion);
  }
...
@Test
  public void shouldAutoRefreshReplicationLagForProject() {
	  ...
	  when(projectVersionRefUpdate.getProjectLocalVersion(eq(projectName))).thenReturn(Optional.of(projectLocalVersion), Optional.of(projectRemoteVersion));
	  when(projectVersionRefUpdate.getProjectRemoteVersion(eq(projectName))).thenReturn(Optional.of(projectRemoteVersion));
