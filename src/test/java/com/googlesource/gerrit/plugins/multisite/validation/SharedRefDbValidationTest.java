...
public class SharedRefDbValidationTest implements RefFixture {
  ...
  private Ref refTwo =
      new ObjectIdRef.Unpeeled(Ref.Storage.NETWORK, "refs/changes/01/1/meta", AN_OBJECT_ID_2);
  private Ref refThree =
      new ObjectIdRef.Unpeeled(Ref.Storage.NETWORK, "refs/changes/01/1/1", AN_OBJECT_ID_3);

  private RemoteRefUpdate remoteRefUpdateOne;
  private RemoteRefUpdate remoteRefUpdateTwo;
  private RemoteRefUpdate remoteRefUpdateThree;
  ...
  public void initCommonMocks() throws IOException {
    ...
    doReturn(AN_OBJECT_ID_3).when(localRepository).resolve(refThree.getName());
    ...
    remoteRefUpdateOne =
        new RemoteRefUpdate(
            localRepository, refOne.getName(), refOne.getName(), true, "bar", refOne.getObjectId());
    remoteRefUpdateTwo =
        new RemoteRefUpdate(
            localRepository, refTwo.getName(), refTwo.getName(), true, "bar", refTwo.getObjectId());

    remoteRefUpdateThree =
        new RemoteRefUpdate(
            localRepository,
            refThree.getName(),
            refThree.getName(),
  ...
  public void remoteRefUpdatesShouldNotBeFilteredOutIfInSyncWithSharedRefDb() throws Exception {
    ...
    doReturn(true).when(sharedRefDb).isUpToDate(eq(A_TEST_PROJECT_NAME), refEquals(refThree));
  ...
  public void remoteRefUpdatesShouldBeFilteredOutIfOutOfSyncWithSharedRefDb() throws Exception {
    ...
    doReturn(false).when(sharedRefDb).isUpToDate(eq(A_TEST_PROJECT_NAME), refEquals(refThree));
  ...
  @Test
  public void remoteRefUpdatesShouldFilterOutChangeImmutableRefs() throws Exception {
    // Refs to push are not the most up to date
    doReturn(false).when(sharedRefDb).isUpToDate(eq(A_TEST_PROJECT_NAME), refEquals(refOne));
    doReturn(false).when(sharedRefDb).isUpToDate(eq(A_TEST_PROJECT_NAME), refEquals(refTwo));
    doReturn(true).when(sharedRefDb).isUpToDate(eq(A_TEST_PROJECT_NAME), refEquals(refThree));
