...
public class ZkSharedRefDatabaseTest extends AbstractDaemonTest implements RefFixture {
  ...
  @Test
  public void aBatchUpdateCanHaveOneSuccessfulAndAOneFailedCommand() throws Exception {
    ObjectId commitObjectIdOne = commitBuilder().add("test_file.txt", "A").create().getId();
    ObjectId commitObjectIdTwo = commitBuilder().add("test_file.txt", "B").create().getId();

    ReceiveCommand firstCommand =
        new ReceiveCommand(ObjectId.zeroId(), commitObjectIdOne, A_REF_NAME_OF_A_PATCHSET);

    ReceiveCommand aNonFastForwardUpdate =
        new ReceiveCommand(
            commitObjectIdOne, commitObjectIdTwo, A_REF_NAME_OF_A_PATCHSET, Type.UPDATE_NONFASTFORWARD);
