...
/**
 * Update project list cache (removals and additions). Cache removals are not forwarded to the other
 * nodes. Cache additions result in a project-created event to be fired, this will notify any
 * registered {@link NewProjectCreatedListener} listeners.
 */
@Singleton
public class ForwardedProjectListUpdateHandler {
   ...
   * handle the event. Any {@link NewProjectCreatedListener} in multi-site will probably want to
   * ignore the event, however to other listeners (eg those defined in HA plugin) this is important
   * as it will forward the update to the other node, and therefore the newly-created project will
   * be visible in the other master node's project list.
   *
