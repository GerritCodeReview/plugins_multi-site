...
public class InMemoryDfsRefDatabase implements DfsRefDatabase, AutoCloseable {
  ...
  InMemoryDfsRefDatabase() {
    this(null, null, 0);
  }

  public InMemoryDfsRefDatabase(
      ScheduledExecutorService cleanerExecutor,
      InMemoryDfsRefDatabaseCleaner cleaner,
      long cleanupIntervalSeconds) {
    refDb = new ConcurrentHashMap<>();

    if(cleanerExecutor != null && cleanupIntervalSeconds > 0) {
      this.cleanerScheduledFuture =
      ...
      this.cleanerScheduledFuture = null;
    }
  }

  @Override
  public void close() {
    if(cleanerScheduledFuture != null) {
  ...
  public boolean updateRefId(String refName, ObjectId newId, ObjectId expectedOldId)
    ...
    final TimedObjectId insertedValue =
        refDb.computeIfPresent(
            refName,
  ...
  public void createRef(String refName, ObjectId id) throws IllegalArgumentException {
    ...
    if (oldValue != null) {
      throw new IllegalArgumentException(
          String.format("Ref '%s' is already in DB with value %s", refName, oldValue.objectId));
    }
