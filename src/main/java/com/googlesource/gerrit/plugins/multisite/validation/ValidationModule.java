...
public class ValidationModule extends FactoryModule {
  ...
  protected void configure() {
    ...
    if (cfg.getZookeeperConfig().isDryRun()) {
      bind(SharedRefEnforcement.class).to(DryRunSharedRefEnforcement.class).in(Scopes.SINGLETON);
    } else if (cfg.getZookeeperConfig().getEnforcementRules().isEmpty()) {
      bind(SharedRefEnforcement.class).to(DefaultSharedRefEnforcement.class).in(Scopes.SINGLETON);
    } else {
      bind(SharedRefEnforcement.class)
          .toInstance(
              new CustomSharedRefEnforcementByProject(
                  cfg.getZookeeperConfig().getEnforcementRules()));
    }
