...
@Singleton
public class Log4jProjectVersionLogger extends PluginLogFile implements ProjectVersionLogger {
  private static final String LOG_NAME = "project_version_log";
  private final Logger verLog;

  @Inject
  public Log4jProjectVersionLogger(SystemLog systemLog, ServerInformation serverInfo) {
  ...
  public void log(NameKey projectName, long currentVersion, long replicationLag) {
    if (replicationLag > 0) {
      verLog.warn("project='{}', version={}, lag={}", projectName, currentVersion, replicationLag);
    } else {
