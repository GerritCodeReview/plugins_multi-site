...
public class RefUpdateUtils {
    ...
    static public Optional <ReceiveCommand> getVersioningCommand(GitRepositoryManager gitRepositoryManager, String project) {
        ReceiveCommand receiveCommand;
        try {
            Repository repository = gitRepositoryManager.openRepository(Project.NameKey.parse(project));
            ObjectInserter ins = repository.newObjectInserter();
            ObjectId newId = ins.insert(OBJ_BLOB, Long.toString(Instant.now().toEpochMilli()).getBytes(UTF_8));
