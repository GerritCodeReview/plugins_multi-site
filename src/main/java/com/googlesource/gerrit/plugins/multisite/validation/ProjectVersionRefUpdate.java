...
public class ProjectVersionRefUpdate implements EventListener {
  ...
  public void onEvent(Event event) {
    ...
    if (!Context.isForwardedEvent() && event instanceof RefUpdatedEvent) {
      ...
      updateLocalVersion(refUpdatedEvent.getProjectNameKey(), refUpdatedEvent.eventCreatedOn);
    }

    // TODO need to catch project created events as well
    if (Context.isForwardedEvent() && event instanceof ChangeEvent) {
      ChangeEvent changeEvent = (ChangeEvent) event;
      updateLocalVersion(changeEvent.getProjectNameKey(), changeEvent.eventCreatedOn);
    }

  }

  private void updateLocalVersion(Project.NameKey projectNameKey, Long creationTimestamp) {
