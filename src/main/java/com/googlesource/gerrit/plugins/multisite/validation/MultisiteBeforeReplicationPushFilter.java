...
public class MultisiteBeforeReplicationPushFilter implements ReplicationPushFilter {
  ...
  public List<RemoteRefUpdate> filterOutOfSyncRemoteRefUpdates(
    ...
    return filterOutOfSyncRemoteRefUpdates(projectName, sharedRefDb, refUpdates.stream())
        .collect(Collectors.toList());
  }

  private Stream<RemoteRefUpdate> filterOutOfSyncRemoteRefUpdates(
      String projectName, SharedRefDatabase sharedRefDb, Stream<RemoteRefUpdate> refUpdates) {

    List<String> changeRefsToRemove = new ArrayList<>();
    List<RemoteRefUpdate> upToDateRefs = new ArrayList<>();
    refUpdates.forEach(
        refUpdate -> {
          try {
    ...
    return upToDateRefs.stream()
        .filter(entry -> !changeRefsToRemove.contains(getTopLevelRefName(entry.getRemoteName())));
  }
