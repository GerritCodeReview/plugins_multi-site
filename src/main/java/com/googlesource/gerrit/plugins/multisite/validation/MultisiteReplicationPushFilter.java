...
public class MultisiteReplicationPushFilter implements ReplicationPushFilter {
  ...
  public List<RemoteRefUpdate> filter(String projectName, List<RemoteRefUpdate> remoteUpdatesList) {
    ...
    try (Repository repository =
      ...
      repLog.error(
          String.format(
              "Error while opening project: '%s'. [Message: %s, Stack Trace: %s]",
              projectName, ioe.getMessage(), Arrays.toString(ioe.getStackTrace())));
  ...
  private Optional<ObjectId> getProjectLocalObjectIdVersion(
    ...
    try {
      ...
      repLog.error(
          String.format(
              "Error while extracting ref '%s' for project: '%s'. [Message: %s, Stack Trace: %s]",
              ref, projectName, ioe.getMessage(), Arrays.toString(ioe.getStackTrace())));
    }
