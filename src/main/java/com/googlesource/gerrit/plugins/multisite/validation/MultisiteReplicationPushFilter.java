  ...
  private boolean isUpToDateWithRetry(
    ...
    try {
      if (sharedRefDb.isUpToDate(
              new Project.NameKey(projectName), new ObjectIdRef.Unpeeled(Ref.Storage.NETWORK, ref, refUpdate.getNewObjectId()))) {
        return true;
      }
      ...
      return sharedRefDb.isUpToDate(
              new Project.NameKey(projectName), new ObjectIdRef.Unpeeled(Ref.Storage.NETWORK, ref, getNotNullExactRef(repository, ref)));
