...
public class InSyncChangeValidator implements RefOperationValidationListener {
  ...
  private List<ValidationMessage> onDeleteRef(Repository repo, RefReceivedEvent refEvent)
      throws ValidationException {
    try {
      Ref oldRef = repo.findRef(refEvent.getRefName());
      if (!dfsRefDatabase.compareAndRemove(refEvent.getProjectNameKey().get(), oldRef)) {
        throw new ValidationException(
            String.format(
    ...
    } catch (NoSuchElementException | IOException refNotInDB) {
  ...
  private List<ValidationMessage> onUpdateRef(Repository repo, RefReceivedEvent refEvent)
    ...
    try {
      ...
      if (!dfsRefDatabase.compareAndPut(refEvent.getProjectNameKey().get(), oldRef, newRef)) {
        throw new ValidationException(
            String.format(
    ...
    } catch (IOException refNotInDB) {
      logger.atSevere().withCause(refNotInDB).log(
          "Local status inconsistent with shared ref database for ref %s. "
              + "Trying to delete it cannot extract the existing one on DB",
          refEvent.getRefName());
