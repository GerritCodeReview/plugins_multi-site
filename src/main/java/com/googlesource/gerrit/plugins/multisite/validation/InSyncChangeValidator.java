...
/**
 * Validates if a change can be applied without bringing the system into a split brain situation
 * by verifying that the local status is aligned with the central status as retrieved by the
 * DfsRefDatabase.
 * It also updates the DB to set the new current status for a ref as a consequence of ref
 * updates, creation and deletions.
 * The operation is done for mutable updates only. Operation on immutable ones are always
 * considered valid.
 */
public class InSyncChangeValidator implements RefOperationValidationListener {
    ...
    public List<ValidationMessage> onRefOperation(RefReceivedEvent refEvent) throws ValidationException {
        ...
        switch (refEvent.command.getType()) {
            ...
            default:
                throw new IllegalArgumentException(String.format("Unsupported command type '%s', in event %s",
        ...
        try {
            if (!dfsRefDatabase.deleteRef(refEvent.getRefName(), refEvent.command.getOldId())) {
                throw new ValidationException(
                        String.format("Unable to delete ref '%s', the old ID '%s' is not equal to the most recent one " +
                                "in the shared ref database", refEvent.getRefName(), refEvent.command.getOldId()));
            }
        } catch (NoSuchElementException refNotInDB) {
            logger.atSevere().withCause(refNotInDB)
                    .log("Local status inconsistent with shared ref database for ref %s. " +
                            "Trying to delete it but it is not in the DB", refEvent.getRefName());

            throw new ValidationException(
                    String.format("Unable to delete ref '%s', cannot find it in the shared ref database",
                            refEvent.getRefName()),
                    refNotInDB);
        }
        return Collections.emptyList();
    ...
    private List<ValidationMessage> onUpdateRef(RefReceivedEvent refEvent) throws ValidationException {
        try {
            if (!dfsRefDatabase.updateRefId(refEvent.getRefName(), refEvent.command.getNewId(), refEvent.command.getOldId())) {
                throw new ValidationException(
                        String.format("Unable to update ref '%s', the old ID '%s' is not equal to the most recent one " +
                                "in the shared ref database", refEvent.getRefName(), refEvent.command.getOldId()));
            }
        } catch (NoSuchElementException refNotInDB) {
            logger.atSevere().withCause(refNotInDB)
            ...
            throw new ValidationException(
                    String.format("Unable to update ref '%s', cannot find it in the shared ref database",
                            refEvent.getRefName()),
                    refNotInDB);
        }
    ...
    private List<ValidationMessage> onCreateRef(RefReceivedEvent refEvent) throws ValidationException {
        try {
            ...
            logger.atSevere().withCause(alreadyInDB)
                    .log("Local status inconsistent with shared ref database for ref %s. " +
                            "Trying to delete it but it is not in the DB", refEvent.getRefName());
