...
public class ZkSharedRefDatabase implements SharedRefDatabase {
    ...
    private boolean applyCreate(String projectName, Ref newRef) throws Exception {
        ...
        final InterProcessMutex refPathMutex = new InterProcessMutex(client, ZkRefInfoMarshaller.pathFor(projectName, newRef));

        try(Locker locker = new Locker(refPathMutex, lockTimeout.toMillis(), MILLISECONDS)) {
            final Optional<ZkRefInfo> zkRefInfoMaybe = marshaller.read(projectName, newRef.getName());

            if(zkRefInfoMaybe.isPresent()) {
                logger.atWarning().log("Asked to create ref %s but it is already in ZK at path %s",
                        newRef.getName(), ZkRefInfoMarshaller.pathFor(projectName, newRef));
                return false;
            }

            marshaller.create(client.transaction(), client.transactionOp(), ZkRefInfo.newInstance(projectName, newRef, instanceId));
    ...
    private boolean applyUpdate(String projectName, Ref oldRef, Ref newRef) throws Exception {
        ...
        try(Locker locker = new Locker(refPathMutex, lockTimeout.toMillis(), MILLISECONDS)) {
            ...
            marshaller.update(ZkRefInfo.newInstance(projectName, newRef, instanceId));
