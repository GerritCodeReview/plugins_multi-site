...
public class SubscriberMetrics extends MultiSiteMetrics {
  ...
  /**
   * Ensures that the generated metric is compatible with prometheus metric names, as the set of values
   * that represent a valid metric name are different from the ones that represent a valid project name.
   * Main differences:
   * - All _ are replaced with __
   * - All characters that aren't a letter(uppercase or lowercase) or a hyphen(-) are replaced with `_<hex_code>`
   * This is to avoid all chances of name clashes when modifying a project name.
   *
