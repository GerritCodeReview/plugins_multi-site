...
public class SubscriberMetrics {
  ...
  public void updateReplicationStatusMetrics(EventMessage eventMessage) {
    ...
    if (event instanceof RefReplicatedEvent) {
      ...
      logger.atInfo().log("Updating replication lag for %s", projectName);
      Optional<Long> remoteVersion =
          projectVersionRefUpdate.getRepositoryRemoteVersion(projectName);
      Optional<Long> localVersion = projectVersionRefUpdate.getRepositoryLocalVersion(projectName);
      if (remoteVersion.isPresent() && localVersion.isPresent()) {
        Long lag = remoteVersion.get() - localVersion.get();
        logger.atInfo().log("Calculated lag for %s [%d]", projectName, lag);
