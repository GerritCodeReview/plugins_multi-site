...
public interface DfsRefDatabase {
    /**
     * Performs a compare and swap atomic operation on the given refName, setting the
     * value for its current ID to the given newId if the value in the DB
     * is equal to the given expected one and the entry hasn't been deleted.
     *
     ...
     * @return true if the operation has been performed successfully and false if it failed
     * because the value is different
     *
     * @throws NoSuchElementException if no entry is found for refName in the DB.
     * Trying to update a deleted entry won't throw this exception but just return false.
     */
    boolean updateRefId(String refName, ObjectId newId, ObjectId expectedOldId) throws NoSuchElementException;
     ...
     * @throws IllegalArgumentException if the entry is already present in the DB (even if in a DELETE state)
     */
     ...
     * Creation of an entry with the same refName will throw an exception.
     * The operation is performed only if the id of the reference is equal to the given one.
     *
